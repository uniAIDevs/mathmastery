/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { LearningPathWhereUniqueInput } from "./LearningPathWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { LearningPathUpdateInput } from "./LearningPathUpdateInput";

@ArgsType()
class UpdateLearningPathArgs {
  @ApiProperty({
    required: true,
    type: () => LearningPathWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => LearningPathWhereUniqueInput)
  @Field(() => LearningPathWhereUniqueInput, { nullable: false })
  where!: LearningPathWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => LearningPathUpdateInput,
  })
  @ValidateNested()
  @Type(() => LearningPathUpdateInput)
  @Field(() => LearningPathUpdateInput, { nullable: false })
  data!: LearningPathUpdateInput;
}

export { UpdateLearningPathArgs as UpdateLearningPathArgs };
