/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ScenarioWhereInput } from "./ScenarioWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ScenarioOrderByInput } from "./ScenarioOrderByInput";

@ArgsType()
class ScenarioFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ScenarioWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ScenarioWhereInput, { nullable: true })
  @Type(() => ScenarioWhereInput)
  where?: ScenarioWhereInput;

  @ApiProperty({
    required: false,
    type: [ScenarioOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ScenarioOrderByInput], { nullable: true })
  @Type(() => ScenarioOrderByInput)
  orderBy?: Array<ScenarioOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ScenarioFindManyArgs as ScenarioFindManyArgs };
